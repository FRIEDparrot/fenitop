...
class DensityFilter():
    def __init__(self, comm, rho, rho_tilde, R=1.0, petsc_options={}):
        ...
        self.vec_s0 = self.rho.x.petsc_vec.copy()
        self.vec_s = self.rho_tilde.x.petsc_vec.copy()
...
class Heaviside():
    ...
    def forward(self, beta, eta=0.5):
        ...
        rho_array = self.rho_phys.x.array
        self.drho = beta*(1-np.tanh(beta*(rho_array-eta))**2) / denominator
        rho_array[:] = (
            np.tanh(beta*eta)+np.tanh(beta*(rho_array-eta))) / denominator
